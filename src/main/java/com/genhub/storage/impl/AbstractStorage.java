...
import com.genhub.storage.Storage;
import com.genhub.utils.FileKit;
import com.genhub.utils.FilePathUtils;
import com.genhub.utils.ImageUtils;
import com.genhub.utils.MD5;
...
public abstract class AbstractStorage implements Storage {
    ...
    protected void validateFile(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new BlogException("File cannot be empty");
        }

        if (!FileKit.checkFileType(file.getOriginalFilename())) {
            throw new BlogException("File format is not supported");
        }
    }
