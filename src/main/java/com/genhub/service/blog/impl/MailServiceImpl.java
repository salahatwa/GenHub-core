...
import com.genhub.service.blog.MailService;
...
//            properties.put("mail.smtp.port", "25");
			...
			log.error(
					"Mail service configuration information is not set, please set it in the background system configuration");
		}
	}
	...
	public void sendTemplateEmail(String to, String title, String template, Map<String, Object> content) {
		...
		taskExecutor.execute(() -> {
			try {
				OhMyEmail.subject(title).from(from).to(to).html(text).send();
